...
int __cdecl pthread_attr_setdetachstate(pthread_attr_t* attr, int state);
int __cdecl pthread_attr_setstacksize(pthread_attr_t* attr, size_t kb);
...
int __cdecl pthread_equal(pthread_t threadA, pthread_t threadB);
int __cdecl pthread_create(
    pthread_t* thread,
    const pthread_attr_t* attr,
    void* (*start_routine)(void*),
    void* arg);
pthread_t __cdecl pthread_self();
int __cdecl pthread_join(pthread_t thread, void** exitCode);

HANDLE __cdecl pthread_getw32threadhandle_np(pthread_t thread);
DWORD __cdecl pthread_getw32threadid_np(pthread_t thread);

int __cdecl pthread_setschedparam(
    pthread_t thread,
    int policy,
...
int __cdecl pthread_mutexattr_settype(pthread_mutexattr_t* attr, int type);
...
int __cdecl pthread_mutex_timedlock(
...
int __cdecl pthread_rwlock_init(pthread_rwlock_t* rwlock, const void* attr);
...
int __cdecl pthread_rwlock_timedrdlock(
...
int __cdecl pthread_rwlock_timedwrlock(
    pthread_rwlock_t* rwlock,
    const timespec* abs_timeout);
...
// it is.
int __cdecl pthread_cond_init(pthread_cond_t* cond, const void* attr);
...
int __cdecl pthread_cond_timedwait(
    pthread_cond_t* cond,
    pthread_mutex_t* mutex,
...
int __cdecl pthread_key_create(pthread_key_t* key, void (*destructor)(void*));
